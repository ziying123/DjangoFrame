...
def send_register_active_email(to_email, username, token):
    ...
    receiver = [to_email]
    html_message = '<h1>%s, 欢迎您成为天天生鲜注册会员</h1>请点击以下链接激活您的账号<br/><a href=' \
                   '"http://127.0.0.1:8000/user/active/%s">http://127.0.0.1:8000/user/active/%s</a>'\
                   % (username, token, token)

    # 发送邮件
    # send_mail 内部函数 只需要穿参数就可  但是需要在 settings 里面做配置
    send_mail(subject, message, sender, receiver, html_message=html_message)
    logger.info("用户：{} 激活邮件发送.".format(username))
